...
def calculate_grid_indices_kernel(pos, grid_indices, N, cell_size, grid_dim_x, grid_dim_y, grid_dim_z):
    ...
    if i < N:
        ...
        # Hash espacial lineal: index = x + y*DIM_X + z*DIM_X*DIM_Y
...
def initialize_particles_kernel(pos, vel, force, pressure, density, N, dim, epsilon, n_per_dim):
    ...
    for i in range(start, N, stride):
        ...
        temp_i = i
        pos[i, 0] = (temp_i % n_per_dim) * epsilon + 1.0 # Offset para no pegar en pared
        temp_i //= n_per_dim
...
def integrate_kernel(pos, vel, force, dt, N, dim):
    i = cuda.grid(1)
    if i < N:
        # --- NUEVO: Límite de velocidad y fuerza ---
        # Esto evita que una sola partícula loca rompa todo el sistema
        MAX_FORCE = 500.0
        ...
        for d in range(dim):
            ...
            elif f < -MAX_FORCE: f = -MAX_FORCE
            
            # 2. Integrar
            vel[i, d] += f * dt
            pos[i, d] += vel[i, d] * dt
            
            # 3. Limpiar fuerza para el siguiente paso
            force[i, d] = 0.0
...
# --- KERNELS CON BÚSQUEDA DE VECINOS  ---
...
@cuda.jit
def apply_wave_generator_kernel(pos, force, current_time, N):
    """
    Aplica una fuerza en una zona específica que varía con el tiempo.
    Actúa como un pistón que genera olas.
    """
    i = cuda.grid(1)
    if i < N:
        # ZONA DE ACTUACIÓN:
        # Solo afectamos a las partículas en el lado izquierdo (X < 0.8)
        # y que estén cerca del fondo (Z < 2.0) para empujar el agua desde abajo.
        if pos[i, 0] < 0.8 and pos[i, 2] < 2.0:
            
            # --- FÓRMULA DE LA PERTURBACIÓN ---
            
            # 1. Frecuencia: Qué tan rápido va y viene la ola (Hz)
            frequency = 30.0 
            
            # 2. Amplitud Base: Fuerza inicial
            base_amplitude = 80.0
            
            # 3. Crecimiento: La fuerza aumenta 10 unidades cada segundo
            growth_factor = 100.0 * current_time 
            
            # Amplitud total actual
            total_amplitude = base_amplitude
            
            # Cálculo de la fuerza oscilatoria (Seno)
            # Empuja en X positivo y negativo (va y viene)
            wave_force = total_amplitude * math.sin(frequency * current_time)
            
            # Aplicar al eje X
            force[i, 0] += wave_force
            
            # Opcional: Un poco de empuje hacia arriba para levantar la ola
            #force[i, 2] += math.abs(wave_force) * 0.1
...
for frame_idx in range(FRAMES):
    ...
    # Obtenemos los índices de ordenamiento
    sorted_indices = cp.argsort(d_grid_indices)
    
    # Reordenamos los arrays de datos para que estén contiguos en memoria
    # NOTA: Esto tiene un costo, pero hace que la lectura de vecinos sea mucho más rápida
    d_pos = d_pos[sorted_indices]
    d_vel = d_vel[sorted_indices]
    d_pressure = d_pressure[sorted_indices] # (Opcional si se recalcula cada frame)
    d_density = d_density[sorted_indices]   # (Opcional)
    ...
    #apply_wave_generator_kernel[blocks_per_grid, threads_per_block](
    #    d_pos, d_force, current_time, N
    #)   
